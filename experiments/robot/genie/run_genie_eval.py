...
def print_and_save(results, sequences, eval_result_path, task_name=None, epoch=None):
    ...
    # model_name = 'vla-test'
    # if not os.path.isdir(f'/cpfs01/user/buqingwen/proj_openvla/{task_name}'):
    #     os.mkdir( f'/cpfs01/user/buqingwen/proj_openvla/{task_name}')
...
def evaluate_policy(model, env, eval_sr_path, eval_result_path, num_procs, procs_id, eval_dir, ep_len, num_sequences, task_name = 'test', debug=False):
    ...
    # with open('/cpfs01/user/buqingwen/proj_openvla/openvla/vla-scripts/enrich_lang_annotations.json', 'r') as f:
...
class GenerateConfig:
    ...
    pretrained_checkpoint: Union[str, Path] = "/cpfs01/user/buqingwen/OmniEmbodiment/vla-scripts/calvin_log/dino-latent-action-pretrained-openvla-bridge-15k+CALVIN_ABC+b16+lr-0.00015+lora-r32+dropout-0.0=w-LowLevelDecoder-ws-12-PCMAP"     # Pretrained checkpoint path
    load_in_8bit: bool = False                       # (For OpenVLA only) Load with 8-bit quantization
    load_in_4bit: bool = False                       # (For OpenVLA only) Load with 4-bit quantization
    
    action_decoder_path:str = "/cpfs01/user/buqingwen/OmniEmbodiment/vla-scripts/calvin_log/dino-latent-action-pretrained-openvla-bridge-15k+CALVIN_ABC+b16+lr-0.00015+lora-r32+dropout-0.0=w-LowLevelDecoder-ws-12-PCMAP/action_decoder-3121.pt"
...
def main(cfg: GenerateConfig) -> None:
    ...
    DATE_TIME_NEW = f'{DATE_TIME}'[:-6]
    save_path = f'/cpfs01/user/buqingwen/OmniEmbodiment/experiments/eval_logs/calvin_{DATE_TIME_NEW}'
    observation_space = {
        'rgb_obs': ['rgb_static', ], 
        'depth_obs': [], 
        'state_obs': ['robot_obs'], 
    ...
    env = make_env('/cpfs01/user/buqingwen/calvin/dataset/task_ABC_D', observation_space, device)
